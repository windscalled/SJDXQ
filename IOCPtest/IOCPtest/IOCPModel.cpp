...
DWORD WINAPI CIOCPModel::_WorkerThread(LPVOID lpParam)
{
	...
	printf("Start worker thread, ID: %d\n", nThreadNo);
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pIOCPModel->m_hShutdownEvent, 0))
	{
		...
		else
		{
			...
			else
			{
				...
				break;
				default:
					printf("_WorkThread's pIoContext->m_OpType is abnormal.\n");
					break;
				}
			}
		}
	...
	printf("Worker thread num%d is quit.\n", nThreadNo);
...
bool CIOCPModel::LoadSocketLib()
{
	...
	if (NO_ERROR != nResult)
	{
		printf("Initialize WinSock 2.2 failed!\n");
		return false;
	}
...
bool CIOCPModel::Start()
{
	...
	{
		printf("IOCP initialization failed!\n");
		return false;
	}
	else
	{
	...
	{
		printf("Listen Socket initialization failed£¡\n");
		this->_DeInitialize();
		return false;
	}
	else
	{
		printf("Listen Socket initialization finished.\n");
	}

	printf("System ready, waiting for connected....\n");
...
bool CIOCPModel::_InitializeIOCP()
{
	...
	{
		printf("IOCP creation failed£¡ERROR num: %d!\n", WSAGetLastError());
		return false;
	}
	...
	printf("Already create %d _WorkerThread in total.\n", m_nThreads);
...
bool CIOCPModel::_InitializeListenSocket()
{
	...
	{
		printf("Socket initialization failed£¬Error num: %d.\n", WSAGetLastError());
		return false;
	}
	else
	{
		printf("WSASocket() finished.\n");
	}

	//Bind Listen Socket to IOCP
	if (NULL == CreateIoCompletionPort((HANDLE)m_pListenContext->m_Socket, m_hIOCompletionPort, (DWORD)m_pListenContext, 0))
	{
		printf("Binding listen Socket to IOCP failed£¡Error num: %d.\n", WSAGetLastError());
		RELEASE_SOCKET(m_pListenContext->m_Socket);
		return false;
	}
	else
	{
		printf("Binding listen Socket to IOCP finished.\n");
	}
	...
	{
		printf("bind() function execution error.\n");
		return false;
	}
	else
	{
	...
	{
		printf("Listen() function execution error.\n");
		return false;
	}
	else
	{
	...
	if (SOCKET_ERROR == WSAIoctl(
		...
		NULL))
	{
		printf("WSAIoctl fail to get AcceptEx function pointer. Error num: %d\n", WSAGetLastError());
		this->_DeInitialize();
		return false;
	}
	...
	if (SOCKET_ERROR == WSAIoctl(
		...
		NULL))
	{
		printf("WSAIoctl fail to get GuidGetAcceptExSockAddrs function pointer. Error num: %d\n", WSAGetLastError());
		this->_DeInitialize();
		return false;
	}
	...
	printf("%d AcceptEx requests posted", MAX_POST_ACCEPT);
...
bool CIOCPModel::_PostAccept(PER_IO_CONTEXT* pAcceptIoContext)
{
	...
	{
		printf("Socket creation failed! Error num: %d", WSAGetLastError());
		return false;
	}
		...
		sizeof(SOCKADDR_IN) + 16, sizeof(SOCKADDR_IN) + 16, &dwBytes, p_ol))
	{
		...
		{
			printf("Post AcceptEx request failed, Error num: %d", WSAGetLastError());
			return false;
		}
	}
...
bool CIOCPModel::_PostRecv(PER_IO_CONTEXT* pIoContext)
{
	...
	{
		printf("Posting first WSARecv failed!\n");
		return false;
	}
	return true;
}
...
bool CIOCPModel::_DoRecv(PER_SOCKET_CONTEXT* pSocketContext, PER_IO_CONTEXT* pIoContext)
{
	...
	printf("Receive %s:%d information£º%s\n", inet_ntoa(ClientAddr->sin_addr), ntohs(ClientAddr->sin_port), pIoContext->m_wsaBuf.buf);
...
bool CIOCPModel::_AssociateWithIOCP(PER_SOCKET_CONTEXT *pContext)
{
	...
	if (NULL == hTemp)
	{
		printf("Executing CreateIoCompletionPort() error.Error num£º%d\n", GetLastError());
		return false;
	}
...
bool CIOCPModel::HandleError(PER_SOCKET_CONTEXT *pContext, const DWORD& dwErr)
{
	...
	if (WAIT_TIMEOUT == dwErr)
	{
		...
		{
			printf("Client disconnect abnormally.\n");
	...
	//may client disconnect abnormally
	...
	{
		printf("IOCP operation error, thread quit. Error num£º%d.\n", dwErr);
		return false;
	}
}